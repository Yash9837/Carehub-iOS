...
struct PatientRecordCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text(patient.name) // Use 'name' instead of 'fullName'
                    .font(.system(size: 18, weight: .semibold))
                    .foregroundColor(.black)
                
                Spacer()
                
                Text("ID: \(patient.patientId)") // Use 'patientId' instead of 'generatedID'
...
struct Records_LT: View {
    ...
    private func loadData() {
        ...
        FirebaseManager.shared.fetchTestResults(forPatientId: "") { tests, error in
            ...
            var fetchedPatients: [PatientInfo] = []
            ...
            group.notify(queue: .main) {
                ...
                self.isLoading = false
                print("Fetched patients: \(self.patients.map { $0.patientId })")
            }
        }
    }
}
...
struct PatientRecordView: View {
    ...
    private func loadTestResults() {
        ...
        FirebaseManager.shared.fetchTestResults(forPatientId: patient.patientId) { results, error in // Use 'patientId'
            isLoading = false
            if let error = error {
